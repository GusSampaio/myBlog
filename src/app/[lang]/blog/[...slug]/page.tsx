    ...
    <BlogPageLayout htmlContent={htmlContent.toString()}>
      ...
      <nav className="mb-8">
        <div className="flex items-center space-x-2 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
          ...
          >
            <BookOpen size={18} />
            <span className="text-sm font-medium">{translationsForLang.navigation.blog}</span>
          </Link>
          ...
          {slugArr.length > 0 && (
            <>
              ...
              {slugArr.map((slug, idx) => {
                const path = `/${lang}/blog/${slugArr.slice(0, idx + 1).join('/')}`;
          ...
          <nav className="mt-8">
            <ul className="space-y-4">
              {subcontents.map(sub => (
                <li key={sub.title}>
                  <Link
                    href={`/${lang}/blog/${[...slugArr, sub.slug].join('/')}`}
